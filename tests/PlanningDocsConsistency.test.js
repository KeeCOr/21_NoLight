...
describe('planning docs implementation status consistency', () => {
  test('planning docs track package version in MD and HTML', () => {
    ...
    const md = read('docs/InkWarrior_기획서.md');
    const html = read('docs/InkWarrior_기획서.html');
  ...
  test('legacy 21NL planning docs do not exist on disk', () => {
    const legacyBaseName = ['21NL', '_기획서'].join('');
    const legacyMdPath = path.join(__dirname, '..', 'docs', `${legacyBaseName}.md`);
    const legacyHtmlPath = path.join(__dirname, '..', 'docs', `${legacyBaseName}.html`);

    expect(fs.existsSync(legacyMdPath)).toBe(false);
    expect(fs.existsSync(legacyHtmlPath)).toBe(false);
  });
});